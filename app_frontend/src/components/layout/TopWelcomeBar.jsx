...
export const TopWelcomeBar = () => {
  const { isDarkMode, t, dailyBriefing, handleGenerateDailyBriefing } = useAppContext();
  ...
  return (
    <GlareHover
      ...
      glareOpacity={0.15}
      className={`${t.card} flex flex-col mb-8 p-6 rounded-2xl transition-all duration-500 gap-6`}
    >
      <div className="flex justify-between items-center w-full relative z-10">
        <div>
          <h1 className={`text-2xl font-bold font-sora ${t.textMain}`}>Welcome back, {firstName}!</h1>
          <p className={`mt-1 ${t.textMuted}`}>Here is your learning progress for today.</p>
        </div>
        <div className={`p-3 rounded-xl flex items-center border ${isDarkMode ? 'bg-yellow-500/10 border-yellow-500/20' : 'bg-yellow-50 border-yellow-100'}`}>
          ...
          <div>
            <p className={`text-sm font-medium ${isDarkMode ? 'text-yellow-300' : 'text-yellow-800'}`}>Next Milestone</p>
      ...
      {showDailyBriefing && (
        <div className={`pt-6 border-t ${t.borderSoft} relative z-20 w-full`}>
          {!dailyBriefing.text && !dailyBriefing.loading ? (
            ...
            <div className={`p-5 rounded-xl shadow-md flex flex-col sm:flex-row gap-4 items-start sm:items-center ${isDarkMode ? 'bg-gradient-to-r from-indigo-900/60 to-blue-900/60 border border-indigo-500/30 text-white' : 'bg-gradient-to-r from-indigo-600 to-blue-600 text-white'}`}>
              ...
              <div className="flex-1">
                <h3 className="font-bold text-lg mb-1">Your Daily Focus</h3>
