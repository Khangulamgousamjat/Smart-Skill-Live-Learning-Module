  ...
  return (
    <div className={`rounded-2xl overflow-hidden glare-hover ${t.card}`}>
      ...
      <div className="p-5">
        ...
        {/* Reason */}
        {req.reason && (
          <div className={`flex gap-2 text-xs p-3 rounded-xl mb-4 ${isDarkMode ? 'bg-white/5 text-slate-300' : 'bg-gray-50 text-gray-600'}`}>
        ...
        {!rejectMode ? (
          ...
          <div className="flex gap-2">
            ...
            <button onClick={() => { setRejectMode(false); setReason(''); }} className={`px-4 text-xs font-semibold rounded-xl border transition-colors ${isDarkMode ? 'border-white/10 text-slate-400 hover:bg-white/5' : 'border-gray-200 text-gray-500 hover:bg-gray-50'}`}>
              Cancel
            </button>
          </div>
        )}
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex justify-between items-center">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            disabled={loading}
            className={`p-2.5 rounded-xl border transition-colors ${isDarkMode ? 'border-white/10 hover:bg-white/5 text-slate-400' : 'border-gray-200 hover:bg-gray-50 text-gray-500'}`}
          >
      ...
      {loading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
          {[1, 2, 3].map(i => (
            <div key={i} className={`h-64 rounded-2xl animate-pulse ${isDarkMode ? 'bg-white/5' : 'bg-gray-100'}`} />
          ))}
        </div>
      ) : requests.length === 0 ? (
